...
class HeatEquationApp:
    ...
    def setup_ui(self):
        ttk.Label(self.root, text="Длина стержня, L (м):").grid(row=0, column=0, padx=5, pady=5, sticky="e")
        ...
        ttk.Label(self.root, text="Коэффициент температуропроводности, a (м²/с):").grid(row=2, column=0, padx=5, pady=5, sticky="e")
        ...
        ttk.Label(self.root, text="Шаг по времени (с):").grid(row=4, column=0, padx=5, pady=5, sticky="e")
        self.dt_entry = ttk.Entry(self.root)
        self.dt_entry.grid(row=4, column=1, padx=5, pady=5)
        self.dt_entry.insert(0, "0.1")
        
        ttk.Label(self.root, text="Общее время, T (с):").grid(row=5, column=0, padx=5, pady=5, sticky="e")
        self.total_time_entry = ttk.Entry(self.root)
        self.total_time_entry.grid(row=5, column=1, padx=5, pady=5)
        self.total_time_entry.insert(0, "10")
        
        self.run_button = ttk.Button(self.root, text="Старт", command=self.start_simulation)
        self.run_button.grid(row=6, column=0, padx=5, pady=5)
        
        self.pause_button = ttk.Button(self.root, text="Пауза", state="disabled", command=self.toggle_pause)
        self.pause_button.grid(row=6, column=1, padx=5, pady=5)
        
        self.figure = plt.Figure(figsize=(8, 5), dpi=100)
        self.ax = self.figure.add_subplot(111)
        self.canvas = FigureCanvasTkAgg(self.figure, master=self.root)
        self.canvas.get_tk_widget().grid(row=0, column=2, rowspan=7, padx=10, pady=10)
